(function() {
    ...
    // ---- 与宿主客户端统一深浅模式的钩子（不影响原逻辑） ----
    window.__navThemeToggle = function () { toggleSetting('theme'); };
    window.__navThemeGet = function () { return themeMode; };
    window.__navThemeDisplay = function () { return getSettingDisplay('theme'); };
    // ---- 宿主关闭导航时同步内部状态（原逻辑不受影响） ----
    window.__navReset = function () { if (state !== 0) closeAll(); else { stopBreathing(); menuLayer.innerHTML = ''; } };
